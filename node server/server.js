...
app.get("/getCategoryQuote", async (req, res) => {
  try {
    ...
    if (rows && rows.length > 0) {
      ...
      res
        .status(404)
        .json({ error: "No quotes found for the specified category" });
    }
  } catch (error) {
...
app.get("/getAuthorQuote", async (req, res) => {
  ...
  try {
    ...
    if (rows && rows.length > 0) {
      ...
      res
        .status(404)
        .json({ error: `No quotes found for the author: ${author}` });
    }
  } catch (error) {
...
app.get("/", (req, res) => {
  res.send(
    "Server is running. Use /getRandomQuote, /getCategoryQuote?category=yourCategory to fetch a quote, /getAuthorQuote?author=yourAuthor, or /getCombinedQuote?author=yourAuthor&category=yourCategory."
  );
});
